...
description: We should be able to efficiently _find the value(min,max or sum etc) from index qs (query start) to qe (query end).
seoDescription: We should be able to efficiently _find the value(min,max or sum etc) from index qs (query start) to qe (query end).
...
image: 'https://prettygood236.github.io/assets/img/data-structures-and-algorithms/range-query/range-query_main.png'
---

<br/>
<img src='https://prettygood236.github.io/assets/img/data-structures-and-algorithms/range-query/range-query_main.png'/>

The Port of Rotterdam, 1907, Paul Signac

We have an array arr[0 . . . n-1]. We should be able to efficiently _<u>find the value(min,max or sum etc) from index qs (query start) to qe (query end)</u>_ where 0 < qs < qe < n-1. <br/>
There are many ways.
...
2. Each internal node represents some <u>merging</u>(min,max or sum etc) of the leaf nodes. For this problem, merging is sum of leaves under a node.
3. An array representation of tree is used to represent Segment Trees. For each node at index **_`i`_**, the left child is at index ***`2*i+1`\***, right child at ***`2*i+2`\*** and the parent is at **_`(i – 1) / 2`_**.

<img src='https://prettygood236.github.io/assets/img/data-structures-and-algorithms/range-query/segment-tree_1.png' width='30%' height='30%'/>
