...
export default function Post({
  ...
  const [comments, setComments] = useState(['첫댓글']);
  ...
  const clockToDateString = (timestamp) =>
    timeAgo.format(new Date(timestamp), 'twitter-now');

  const postDetail = async () => {
    // const result = await viewDetail(index, post);

    setComments([comment, ...comments]);
  };

  const createCommentForPost = async (text) => {
    createComment(text, index, commentCount);
  };
    ...
    <div className={style.wrapper}>
      <div className={style.postPublisher}>
        ...
        <div className={style.publisherDetails}>
          <div className={style.name}>{posterName}</div>
          <div className={style.timestamp}>{clockToDateString(postTime)}</div>
        </div>
      </div>
